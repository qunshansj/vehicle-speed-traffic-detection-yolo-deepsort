...
3.根据实际问题优化模型。
# 4.图片展示
![4.png](0ea2447e51d7b63d9241f537b3591b1b.png)

![5.png](d4936cf22fc4853bb51b073be779c73d.png)


# 5.视频展示
[[YOLOv7]基于YOLO＆Deepsort的车速＆车流量检测系统(源码＆部署教程)_哔哩哔哩_bilibili](https://www.bilibili.com/video/BV1DW4y1q7TM/?vd_source=bc9aec86d164b67a7004b996143742dc)
...
#### sort工作流程如下图所示：
![4.png](df8dc63d2bce19f20a76702c6261f9d6.png)
...
#### Deepsort算法流程
 由于sort算法还是比较粗糙的追踪算法，当物体发生遮挡的时候，特别容易丢失自己的ID。而[参考该博客改进后的算法在sort算法的基础上增加了级联匹配（Matching Cascade）和新轨迹的确认（confirmed）](https://mbd.pub/o/bread/Y5WZlpZy)。Tracks分为确认态（confirmed），和不确认态（unconfirmed），新产生的Tracks是不确认态的；不确认态的Tracks必须要和Detections连续匹配一定的次数（默认是3）才可以转化成确认态。确认态的Tracks必须和Detections连续失配一定次数（默认30次），才会被删除。
Deepsort算法的工作流程如下图所示：
![5.png](90866c5f7ad19819c8ae117614a69206.png)
 整个算法的工作流程如下：
...
# 7.准备YOLOv7格式数据集
如果不懂yolo格式数据集是什么样子的，[建议先学习一下该博客](https://afdian.net/item?plan_id=20601d48623211ed976d52540025c377)。大部分CVer都会推荐用labelImg进行数据的标注，我也不例外，推荐大家用labelImg进行数据标注。不过这里我不再详细介绍如何使用labelImg，网上有很多的教程。同时，标注数据需要用到图形交互界面，远程服务器就不太方便了，因此建议在本地电脑上标注好后再上传到服务器上。

这里假设我们已经得到标注好的yolo格式数据集，那么这个数据集将会按照如下的格式进行存放。
![n.png](23b21da169a6240f47ff4863696fe3c5.png)
...
![b.png](e59eb48a5d67f38878dfba0a7205b0b4.png)
...
![k.png](fda151762669e6f8c55adde1d7846012.png)
...
```
# 11.车速检测
车辆速度是交通信息采集系统检测的主要动态参数之一,对超速违章车辆进行监测进而限速以消除交通安全隐患也日趋重要.论文对当今各种车速检测技术进行了分类研究,阐述了各检测技术的原理并对其性能进行了综合比较和评价,特别提出了一种基于磁通门技术的检测技术.最后,对各检测技术实际的应用场合及情况进行了比较说明,表明视频车辆检测技术将是未来实时交通信息采集和处理技术的发展方向。
#### 代码实现
```
...
# 12.系统整合
下图[完整源码&环境部署视频教程&自定义UI界面](https://s.xiaocichang.com/s/6e8e4e)
![1.png](f475428b307c0f7b5c267c4d0fec388b.png)

参考博客[《\[YOLOv7\]基于YOLO＆Deepsort的车速＆车流量检测系统(源码＆部署教程)》](https://mbd.pub/o/qunma/work)
...
# 13.参考文献:
***
*   *[1]*[机动车视频测速中关键技术的研究与实现](https://kns.cnki.net/kcms/detail/detail.aspx?filename=JSJC200605069&dbcode=cjfd&dbname=cjfd2006&v=iCFftG9-iazYlOxj0ubZVteDB6KecdRzjuEKLzuRc-O6DEaLEqdlGSA7gzP59DwO)[J]. 王命延,朱明峰,王昊.  计算机工程. 2006(05)
*   *[2]*[超声波原理与现代应用探讨](https://kns.cnki.net/kcms/detail/detail.aspx?filename=GZDI200503013&dbcode=CJFD&dbname=CJFD2005&v=lm20FgkORtQ7xTKYfydSNNBFS58MyqLuujZ0SfKmaeeFSPZNEOjBjM9UVY7vMJOZ)[J]. 王育慷.  贵州大学学报(自然科学版). 2005(03)
*   *[3]*[地感线圈在交通控制领域中的应用](https://kns.cnki.net/kcms/detail/detail.aspx?filename=ELEW200508027&dbcode=CJFD&dbname=CJFD2005&v=OhF5l9Tk97o_yATwJ0TNYm-atCCYiAnn2a28h31e6yDxzrjNm1cAZSlTGDsOPraN)[J]. 王树欣 ,伍湘彬.  电子世界. 2005(08)
*   *[4]*[公路动态称重系统红外测速传感器的研制](https://kns.cnki.net/kcms/detail/detail.aspx?filename=JSYZ200506073&dbcode=CJFD&dbname=CJFD2005&v=vD0PtUGgMhJN4pCyKm7AUShFSi00tj8JLyFIU45-2OVPhWuxWKfCAAjQ3Elmge3Z)[J]. 苏秀平,王秀芳.  机械设计与制造. 2005(06)
*   *[5]*[脉冲式半导体激光测速仪的研究](https://kns.cnki.net/kcms/detail/detail.aspx?filename=CDXB200502027&dbcode=CJFD&dbname=CJFD2005&v=Ul_sblRsZv0H_8Akq3wHxl7vD1M29msq9hV7MWPFYUtB2ZqERrP78mv9KzhxD1dP)[J]. 周晶.  长春大学学报. 2005(02)
*   *[6]*[雷达测速在公安交通管理中的应用](https://kns.cnki.net/kcms/detail/detail.aspx?filename=GDGA200501023&dbcode=CJFD&dbname=CJFD2005&v=G2zo1QXYSufh_Bg5-R_mT_yBIUJTrel87rFyJUe7P3oN1X4LBjJjBmC0TKgqZhNk)[J]. 杨粤湘.  广东公安科技. 2005(01)
*   *[7]*[激光测速的物理学原理](https://kns.cnki.net/kcms/detail/detail.aspx?filename=XDWZ199805003&dbcode=CJFD&dbname=CJFD1998&v=CzXsHD93LTkfEFrFeGx8JFnKY_nVudeAduZatxA6hLvuKpix8KkQsUxCXN_3zRDR)[J]. 仇九子.  现代物理知识. 1998(05)
